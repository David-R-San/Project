...
# Uncomment to start training from checkpoint:
model.load_state_dict(torch.load(r"C:/Users/David/Downloads/chex_clinical_project/saved_models/checkpoint_epoch_3_melhor.pth", map_location=device))
...
pd.DataFrame(loss_history).to_csv("results/loss_per_epoch.csv", index=False)
...
pd.DataFrame([summary]).to_csv("results/final_metrics.csv", index=False)
